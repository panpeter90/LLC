...
void test( char* argv[] )
{
	...
	if ( fs.isOpened() )  fs["vocabulary"] >> vocabulary;
        ...
        for ( vector<string>::iterator itr = imgs_fns.begin(); itr != imgs_fns.end();  itr++)
        {
				...
				std::cout << "Time spent detect:" <<float( clock () - begin_time1 ) /  CLOCKS_PER_SEC << "\n" << endl;
				...
				std::cout << "Time spent llc_bow_Descriptor:" <<float( clock () - begin_time ) /  CLOCKS_PER_SEC << "\n" << endl;
...
int main( int argc, char* argv[] )
{
	...
	argv[1] = "data/train";
	argv[2] = "Bicycle";
	argv[3] = "data/imgs/Bicycle";
	argv[4] = "data/resultsvms";
	argv[5] = "data/result/vocabulary.xml";
	//test  sunflower  ../data/imgs/sunflower  ../data/result/svms  ../data/result/vocabulary.xml butterfly hawksbill watch
	test(argv);
...
void help( const char* progName )
{
	...
	std::cout << "  example: " << progName << " test  sunflower  ../data/imgs/sunflower  ../data/result/svms  ../data/result/vocabulary.xml" << std::endl;
}
